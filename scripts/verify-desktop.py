...
require(conf['version']=='0.9.0','Tauri version mismatch')
pkg=json.loads((root/'package.json').read_text())
require(pkg['version']=='0.9.0','package version mismatch')
require(pkg['name']=='openguin-preview','npm package must use Openguin branding')
for step in ['apply-build-fixes.py','apply-full-logs.py','apply-observatory.py','apply-performance09.py','apply-openguin-brand.py','ensure-app-icon.py','prepare-ollama-sidecar.sh']:
...
require('version = "0.9.0"' in cargo,'Rust version mismatch')
lib=(root/'src-tauri/src/lib.rs').read_text()
for token in ['runtime_discovery','start_bundled_ollama','stop_bundled_ollama','ollama_json','pull_model','search_huggingface','list_hf_gguf_variants','import_hf_gguf','system_profile','127.0.0.1:11435','127.0.0.1:11434','runtime_dir','runtime_complete','OLLAMA_LIBRARY_PATH','llama-server','/api/generate','/api/ps']:
...
for token in ['RUNTIME CONTROL','CONTEXT OPTIMIZER','keep_alive','Preload','Unload','recommended','ceiling']:
    require(token in rt,f'Missing runtime control token: {token}')
cmp=(root/'src/CompareBench09.tsx').read_text()
for token in ['MODEL COMPARATOR','COLD / WARM BENCHMARK','MEMORY HISTORY','openguin-coldwarm','load_duration','eval_duration']:
    require(token in cmp,f'Missing comparison/benchmark token: {token}')
for file in ['src/observatory.css','src/performance09.css','LICENSE','NOTICE.md','THIRD_PARTY_NOTICES.md','COPYRIGHT.md','SECURITY.md','CONTRIBUTING.md','docs/ENGINE_BEHAVIOR.md','docs/MODEL_LICENSING.md']:
...
if errors:
    ...
    sys.exit(1)
print('Desktop verification OK — Openguin 0.9')
...
print(' - conservative Context Optimizer wired')
print(' - side-by-side model comparison wired')
