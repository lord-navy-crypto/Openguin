...
if 'OPENGUIN_011_STATIC_RUST_COMPOSITION' in text:
    for required in [
        ...
        if required not in text:
            raise SystemExit(f'apply-full-logs: static Rust diagnostics integration missing: {required}')
else:
...
ui=app.read_text()
static_ui='OPENGUIN_011_STATIC_COMPOSITION' in ui
if static_ui:
    ...
    if 'OPENGUIN_011_STATIC_RUST_COMPOSITION' in text:
        print('Diagnostics patch skipped: Rust command handler and three-layer UI logs are static and verified.')
    else:
        print('Static diagnostics UI verified; legacy Rust command registration prepared.')
    raise SystemExit(0)
...
app.write_text(ui)
print('Applied legacy full diagnostics integration.')
