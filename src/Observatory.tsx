...
export default function Observatory({mode,memoryBytes,installedCount,online}:Props){
 ...
 const totalVram=active.reduce((s,m)=>s+(m.size_vram??0),0),latest=bench[0],recent=[...bench].slice(0,20).reverse();
 const avg=bench.length?bench.reduce((s,b)=>s+(b.tokS||0),0)/bench.length:0;
 return <div className="obs-page">
  ...
  <div className="obs-two">
   ...
   <section className="obs-card"><h3>Runtime memory map</h3><p className="obs-muted">Loaded model memory compared with unified/system memory.</p><div className="memory-donut" style={{'--p':`${pct(totalVram,memoryBytes)}%`} as React.CSSProperties}><div><b>{pct(totalVram,memoryBytes).toFixed(0)}%</b><span>allocated</span></div></div><div className="memory-legend"><span><i/>Models <b>{fmt(totalVram)}</b></span><span><i/>Available headroom <b>{fmt(Math.max(0,memoryBytes-totalVram))}</b></span></div><p className="obs-note">On Apple Silicon, CPU and GPU share unified memory, so this view deliberately compares Ollama's reported model allocation against total unified memory instead of pretending there is separate VRAM.</p></section>
  </div>
  <div className="obs-two">
   <section className="obs-card"><h3>Decode performance trend</h3><p className="obs-muted">Measured output-token throughput from recent Model Lab runs.</p><Spark rows={recent}/><div className="trend-footer"><span>slowest <b>{bench.length?Math.min(...bench.map(b=>b.tokS)).toFixed(1):'—'}</b></span><span>average <b>{avg?avg.toFixed(1):'—'}</b></span><span>fastest <b>{bench.length?Math.max(...bench.map(b=>b.tokS)).toFixed(1):'—'}</b> tok/s</span></div></section>
   <section className="obs-card"><h3>Last generation pipeline</h3><p className="obs-muted">Load → prompt prefill → token decode. Values appear after a new 0.8 telemetry-enabled run.</p>{latest?<Pipeline b={latest}/>:<div className="obs-empty">No benchmark telemetry yet.</div>}</section>
  </div>
