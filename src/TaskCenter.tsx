...
export default function TaskCenter(){
 ...
 async function cancelOrDismiss(r:Row){
  const activeState=['queued','running','stalled'].includes(r.state);
 ...
 return <div className={`task-center ${open?'open':'closed'}`}><button className="task-toggle" onClick={()=>setOpen(v=>!v)} aria-expanded={open}><span className={active.length?'pulse':''}/><b>Tasks</b><em>{active.length}</em></button>{open&&<section className="task-panel"><header><div><b>Activity</b><small>{active.length?`${active.length} active task${active.length===1?'':'s'}`:'All tasks complete'}</small></div><button onClick={clearFinished} disabled={!done.length}>Clear finished</button></header><div className="task-list">{shown.length?shown.map(r=>{const activeState=['queued','running','stalled'].includes(r.state);const realCancel=activeState&&r.cancellable&&r.cancelKind&&r.cancelTarget;return <article key={r.id} className={`task-row ${r.state}`}><div className="task-top"><div><b>{r.title}</b><small>{r.source||'Openguin'} · {elapsed(now()-r.createdAt)}</small></div><span>{r.state}</span></div><div className="task-detail">{r.detail||'Working…'}</div><div className={`task-progress ${r.progressKind==='indeterminate'?'indeterminate':''}`}><i style={r.percent===undefined?undefined:{width:`${r.percent}%`}}/></div><div className="task-bottom"><small>{r.progressKind==='real'?'Measured progress':r.progressKind==='stage'?'Stage progress':'Waiting for progress data'}{r.state==='stalled'?' · no update for 15s':''}</small><div>{r.percent!==undefined&&<b>{Math.round(r.percent)}%</b>}<button onClick={()=>cancelOrDismiss(r)}>{realCancel?'Cancel':activeState?'Dismiss':'Remove'}</button></div></div></article>}):<div className="task-empty">No tasks yet. Long-running actions will appear here as soon as you click them.</div>}</div></section>}</div>
}
