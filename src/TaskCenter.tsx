...
const isActive=(s:TaskState)=>['queued','running','stalled'].includes(s);
function loadRows():Row[]{try{const raw=JSON.parse(localStorage.getItem(STORE)||'[]');if(!Array.isArray(raw))return[];return raw.slice(0,40).map((r:Row)=>isActive(r.state)?{...r,state:'stalled' as TaskState,detail:`${r.detail||'Task interrupted'} · restored after UI restart`,cancellable:false,updatedAt:r.updatedAt||now()}:r)}catch{return[]}}
