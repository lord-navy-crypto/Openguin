...
export default function RuntimeControl09({mode,memoryBytes}:{mode:Mode;memoryBytes:number}){
 ...
 async function refresh(){try{const[t,p]=await Promise.all([api(mode,'GET','/api/tags'),api(mode,'GET','/api/ps')]);const nextModels=t.models??[],nextActive=p.models??[];setModels(nextModels);setActive(nextActive);setTarget(v=>v||nextModels[0]?.name||'');return nextActive as Active[]}catch(e){setStatus(String(e));return null}}
