...
printf '\nOpenguin 0.10.0 Development Launcher\n====================================\n'
missing=0
for cmd in node npm rustc cargo curl ditto python3; do
  if ! command -v "$cmd" >/dev/null 2>&1; then echo "Missing: $cmd"; missing=1; fi
done
if [ "$missing" -ne 0 ]; then
  echo
  echo "Install the missing development tools, then run this launcher again."
  echo "Ollama itself does NOT need to be installed separately; Overview can repair/download the private runtime."
  exit 1
fi
